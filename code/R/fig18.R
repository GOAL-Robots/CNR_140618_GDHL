...
all_plots <- function() {
    # fig 18 - Graph of 9 G-DHL kernels, including Hebb ----
    layout(matrix(c(1, 2, 3, 4, 5, 6, 7, 8, 9), 3, 3, byrow = TRUE)) #Creates graphical matrix spots where to put graps. Graph with order 1 in code will occupy slots with index 1: graphs with order 2 will occupy slots with index 2, ...
    par(mar = c(4., 4, 1.5, 1.5), mgp=c(2, 1, 0))
    plot(
    ...
    plot(
        seq(-sEvenDura, sEvenDura, sDeltTime),
        append(mWeigSS[, iSimuStepNumb], 0, after = iDelaNumb),
        "l",
        xlab = "Inter-event interval (sec)",
        ylab = "Weight update",
        cex.main=0.8,
