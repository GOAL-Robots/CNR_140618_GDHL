...
# Figure 1 - Combination of kernels: Hebb, Kosko, and Worgotter kernels ----
mWeigHebb = matrix(rep(0.0,iDelaNumb*iSimuStepNumb),iDelaNumb,iSimuStepNumb,TRUE)
mWeigKosk = matrix(rep(0.0,iDelaNumb*iSimuStepNumb),iDelaNumb,iSimuStepNumb,TRUE)
mWeigWorg = matrix(rep(0.0,iDelaNumb*iSimuStepNumb),iDelaNumb,iSimuStepNumb,TRUE)
mWeigHebb = 0 * mWeigPP + 0 * mWeigSP + 0 * mWeigNP + 0 * mWeigPS + 1 * mWeigSS + 0 * mWeigNS + 0 * mWeigPN + 0 * mWeigSN + 0 * mWeigNN
mWeigKosk = 1 * mWeigPP + 0 * mWeigSP - 1 * mWeigNP + 0 * mWeigPS + 0 * mWeigSS + 0 * mWeigNS - 1 * mWeigPN + 0 * mWeigSN + 1 * mWeigNN
mWeigWorg = 0 * mWeigPP + 1 * mWeigSP + 0 * mWeigNP + 0 * mWeigPS + 0 * mWeigSS + 0 * mWeigNS + 0 * mWeigPN - 1 * mWeigSN + 0 * mWeigNN


all_plots <- function() {
    layout(matrix(c(1,2,3), 1, 3, byrow=TRUE)) #Creates graphical matrix spots where to put graps. Graph with order 1 in code will occupy slots with index 1: graphs with order 2 will occupy slots with index 2, ...
    par(mar= c(5, 4.5, 1, 0.5))

    plot(
        seq(-sEvenDura, sEvenDura, sDeltTime),
        append(mWeigHebb[, iSimuStepNumb], 0, after = iDelaNumb),
        "l",
    ...
    axis(1, at=c(-1.0, 0.0, 1.0), labels=format(c(-1.0, 0.0, 1.0), nsmall = 1))

    plot(
        seq(-sEvenDura, sEvenDura, sDeltTime),
        append(mWeigKosk[, iSimuStepNumb], 0, after = iDelaNumb),
        "l",
        xlab = "Inter-event interval (sec)",
        ylab = "Weight update",
        main = "Kosko rule",
    ...
    plot(
        seq(-sEvenDura, sEvenDura, sDeltTime),
        append(mWeigWorg[, iSimuStepNumb], 0, after = iDelaNumb),
        "l",
        xlab = "Inter-event interval (sec)",
        ylab = "Weight update",
        main = "Woergoetter rule",
        cex.main=0.8,
        xaxt="n"
    )
    axis(1, at=c(-1.0, 0.0, 1.0), labels=format(c(-1.0, 0.0, 1.0), nsmall = 1))
