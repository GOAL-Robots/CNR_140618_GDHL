...
#Figure 2 - Graph including all 5 unique/non-repeated kernels ----

all_plots <- function() {
    par(mar= c(5, 4, 1, 2))
    x = seq(-sEvenDura, sEvenDura, sDeltTime)
    y = append(mWeigPP[, iSimuStepNumb], 0, after = iDelaNumb)
    ...
    par(new = TRUE)
    text(x[which.max(y)], max(y) + 0.2, "PP")
    par(new = TRUE)

    y = 2.8 * append(mWeigSP[, iSimuStepNumb], 0, after = iDelaNumb)
    plot(
        x,
        y,
        "l",
    ...
    par(new = TRUE)
    text(x[which.max(y)], max(y) + 0.2, "SP/NS")

    par(new = TRUE)

    y = append(mWeigNP[, iSimuStepNumb], 0, after = iDelaNumb)
    plot(
        x,
        y,
        "l",
    ...
    text(x[which.max(y)], max(y) + 0.2, "NP")
    par(ps = 11,new = TRUE)

    y = 2.8 * append(mWeigPS[, iSimuStepNumb], 0, after = iDelaNumb)
    plot(
        x,
        y,
        "l",
    ...
    text(x[which.max(y)], max(y) + 0.2, "PS/SN")
    par(ps = 11,new = TRUE)

    y = append(mWeigPN[, iSimuStepNumb], 0, after = iDelaNumb)
    plot(
        x,
        y,
        "l",
    ...
    text(x[which.max(y)], max(y) + 0.2, "PN")
    par(new = TRUE)
}
