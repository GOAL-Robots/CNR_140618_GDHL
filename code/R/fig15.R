...
vWeigChanB = (vSign1 * vSign2b) * sDeltTime
vWeigB = cumsum(vWeigChanB)

vWeigChanS = rep(0, sSimuStepNumb)
vWeigChanS = (vSign1 * vSign2s) * sDeltTime
vWeigS = cumsum(vWeigChanS)

vWeigChanA = rep(0, sSimuStepNumb)
vWeigChanA = (vSign1 * vSign2a) * sDeltTime
vWeigA = cumsum(vWeigChanA)
sYRange <- range(vSign1) #Compute min and max of y axis
vSignPre = vSign1
mSignPost = matrix(c(vSign2b, vSign2s, vSign2a), 3, sSimuStepNumb, TRUE) #Matrix of 3 signals at different times

#*********************************************
#GRAPHICS

all_plots <- function() {
    #Extra plot of a signal with its positive derivative part and negative derivative part
    par(mar = c(3.5, 3, 0.5, 0.5)) #Set respectively bottom, left, top, right margins
    sXRange <- c(0, 2.3)
    sYRange <-
        range(c(vSignPre, (vSign1Deri * (vSign1Deri > 0)))) #Compute min and max of y axis
    plot(
        vTime,
        vSignPre,
        "l",
    ...
    legend(
        "topright",
        legend = c("Signal", "Positive der.", "Negative der."),
        lty = c(1, 1, 2),
        lwd = c(1, 3, 1),
