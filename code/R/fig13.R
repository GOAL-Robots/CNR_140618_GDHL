...
vWeigChanB = (vSign1Deri * vSign2bDeri) * sDeltTime
vWeigB = cumsum(vWeigChanB)

vWeigChanS = rep(0,sSimuStepNumb)
vWeigChanS = (vSign1Deri * vSign2sDeri) * sDeltTime
vWeigS = cumsum(vWeigChanS)

vWeigChanA = rep(0,sSimuStepNumb)
vWeigChanA = (vSign1Deri * vSign2aDeri) * sDeltTime
vWeigA = cumsum(vWeigChanA)

sYRange<-range(c(vSign1Deri, vSign2bDeri,vSign2sDeri,vSign2aDeri)) #Compute min and max of y axis, based on min/max values of vSign1 and vSing2
vSignPre = vSign1Deri
